...
def gen_image(args: argparse.Namespace, file_list: list) -> None:
    ...
    for filename in tqdm(file_list):
        ...
        for slice in tqdm(range(image_tensor.shape[0]), leave=False):
            ...
            pil_image.save(f"/Dataset/maki/brats/Task01_BrainTumour/{args.path}/images/{os.path.basename(filename)}_slice{slice}.png")

    with open(f"/Dataset/maki/brats/Task01_BrainTumour/{args.path}/images/slices.pkl", "wb") as f:
...
def main():
    ...
    if os.path.isdir(f"/Dataset/maki/brats/Task01_BrainTumour/{args.path}"):
        os.makedirs(f"/Dataset/maki/brats/Task01_BrainTumour/{args.path}/images", exist_ok=True)
        file_list = [file for file in glob.glob(f"/Dataset/maki/brats/Task01_BrainTumour/{args.path}/**/*.nii.gz", recursive=True) if os.path.isfile(file)]
    else:
        raise ValueError(f"path: /Dataset/maki/brats/Task01_BrainTumour/{args.path} is not directory.")
